...
showSavedBillPayment = function(billId) {
  ...
  openModal(`<h2>Bayar ${esc(bill.label)}</h2><p>${state.orderIds[cartKey()]} · Meja ${state.table} · <strong>${money(sum.total)}</strong></p><div class="pending-bill-preview">${bill.items.map(i => `<div class="total-row"><span>${i.qty}× ${esc(i.name)}</span><strong>${money(i.price * i.qty)}</strong></div>`).join('')}</div><div class="payment-grid">${[['cash', '💵'], ['card', '💳'], ['qris', '▦']].map(([key, icon]) => `<button data-saved-payment="${key}">${icon}<span>${t(key)}</span></button>`).join('')}</div><div class="modal-actions"><button class="secondary" id="backSavedBills">Kembali</button></div>`);
...
receiptHTML = function(transaction, index = 0, copyType = 'customer') {
  ...
  return `<div class="receipt" data-receipt="${index}">${logo}<h2>${esc(state.settings.restaurantName)}</h2><p style="text-align:center"><strong>${status}</strong>${copy}<br>${esc(transaction.id)}<br>${esc(transaction.orderId)} · ${esc(transaction.time)}<br>${esc(transaction.table)}${transaction.splitPerson ? ` · Orang ${transaction.splitPerson}` : ''}<br>Waiter: ${esc(transaction.waiter || '—')}<br>Shift: ${esc(transaction.shiftName || transaction.shiftId || '—')}</p>${(transaction.lineItems || []).map(x => `<div class="receipt-line"><span>${x.qty}× ${esc(x.name)}${x.taxable ? '' : ' *'}</span><span>${money(x.lineTotal)}</span></div>`).join('')}<div class="receipt-line receipt-total"><span>Subtotal</span><span>${money(transaction.subtotal || 0)}</span></div>${transaction.discountAmount ? `<div class="receipt-line"><span>${esc(transaction.discountLabel || 'Diskon')}</span><span>-${money(transaction.discountAmount)}</span></div>` : ''}<div class="receipt-line"><span>Dasar kena pajak</span><span>${money(transaction.taxableSubtotal || 0)}</span></div><div class="receipt-line"><span>${dynamicTaxLabel(transaction.taxRate ?? 10)}</span><span>${money(transaction.tax || 0)}</span></div><div class="receipt-line"><strong>Total</strong><strong>${money(transaction.total || 0)}</strong></div>${transaction.paymentCode === 'cash' && transaction.tendered !== null ? `<div class="receipt-line"><span>Uang tamu</span><span>${money(transaction.tendered)}</span></div><div class="receipt-line"><span>Kembalian</span><span>${money(transaction.change || 0)}</span></div>` : ''}<p>* Tanpa pajak</p><p style="text-align:center">${esc(transaction.payment || '')}<br>Kasir: ${esc(transaction.cashier || state.user.name)}<br><br>Terima kasih · Thank you</p></div>`;
};
...
showTestReceipt = function() { const now = new Date(), rate = currentTaxRate(), taxableSubtotal = 100000, tax = Math.round(taxableSubtotal * rate / 100), sample = { id: 'TEST-80MM', orderId: 'ORD-TEST', status: 'closed', time: now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }), table: 'Meja Uji', waiter: 'Nama Waiter', cashier: state.user.name, shiftId: state.shift.id || 'SFT-TEST', shiftName: state.shift.name || 'Shift Uji', payment: 'TEST PRINT', subtotal: 150000, discountAmount: 0, taxableSubtotal, taxRate: rate, tax, total: 150000 + tax, lineItems: [{ qty: 1, name: 'Nasi Goreng Spesial', taxable: true, lineTotal: 100000 }, { qty: 1, name: 'Minuman Tanpa Pajak', taxable: false, lineTotal: 50000 }] }; openModal(`${receiptHTML(sample)}<div class="modal-actions receipt-actions"><button class="secondary close-modal">Tutup</button><button class="primary" id="printReceipt">Cetak uji 80 mm</button></div>`); $('#printReceipt').onclick = () => printReceipt(); };
