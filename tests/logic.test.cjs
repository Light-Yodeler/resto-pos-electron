...
test('Windows thermal raster renders at 4x zoom for high-res 203 DPI output',()=>{const fs=require('node:fs'),path=require('node:path'),main=fs.readFileSync(path.join(__dirname,'..','electron','main.cjs'),'utf8'),script=fs.readFileSync(path.join(__dirname,'..','electron','print-windows.ps1'),'utf8'),block=main.slice(main.indexOf('async function printThermalReceipt'),main.indexOf('function createWindow'));assert.match(block,/RENDER_SCALE = 4/);assert.match(block,/setZoomFactor/);assert.match(block,/setContentSize/);assert.match(block,/scrollH1x/);assert.match(block,/DOTS_PER_MM/);assert.match(script,/SetThreshold\(0\.58\)/);assert.match(script,/HighQualityBicubic/)});
