...
test('Windows raster captures only receipt bounds to avoid double scaling',()=>{const source=require('node:fs').readFileSync(require('node:path').join(__dirname,'..','electron','main.cjs'),'utf8'),block=source.slice(source.indexOf('async function printThermalReceipt'),source.indexOf('function createWindow'));assert.match(block,/querySelector\('\.receipt'\)\.getBoundingClientRect/);assert.match(block,/capturePage\(receiptBounds\)/);assert.equal(block.includes('capturePage();'),false)});
test('receipt supports thin monospace, medium, and bold typography choices',()=>{const main=require('node:fs').readFileSync(require('node:path').join(__dirname,'..','electron','main.cjs'),'utf8'),app=require('node:fs').readFileSync(require('node:path').join(__dirname,'..','app','app.js'),'utf8');assert.match(main,/fontStyle === 'bold'/);assert.match(main,/fontStyle === 'medium'/);assert.match(main,/Consolas, "Courier New", monospace/);assert.match(app,/receiptFontStyle:'clear'/);assert.match(app,/Tipis & jelas · Monospace/)});
test('Windows thermal raster preserves receipt scale and uses balanced monochrome threshold',()=>{const fs=require('node:fs'),path=require('node:path'),main=fs.readFileSync(path.join(__dirname,'..','electron','main.cjs'),'utf8'),script=fs.readFileSync(path.join(__dirname,'..','electron','print-windows.ps1'),'utf8'),block=main.slice(main.indexOf('async function printThermalReceipt'),main.indexOf('function createWindow'));assert.equal(block.includes('setZoomFactor'),false);assert.match(script,/SetThreshold\(0\.58\)/);assert.match(script,/HighQualityBicubic/)});
...
test('cut feed distance defaults to eight lines and is configurable',()=>{const app=require('node:fs').readFileSync(require('node:path').join(__dirname,'..','app','app.js'),'utf8');assert.match(app,/cutFeedLines:8/);assert.match(app,/name="cutFeedLines"/);assert.match(app,/8 baris · Aman/);assert.match(app,/cutFeedLines:Number\(state\.settings\.cutFeedLines\)\|\|8/)});
