...
test('Windows thermal raster upscales 96 DPI capture to 203 DPI printer dots in dither loop',()=>{const fs=require('node:fs'),path=require('node:path'),main=fs.readFileSync(path.join(__dirname,'..','electron','main.cjs'),'utf8'),script=fs.readFileSync(path.join(__dirname,'..','electron','print-windows.ps1'),'utf8'),block=main.slice(main.indexOf('async function printThermalReceipt'),main.indexOf('function createWindow'));assert.match(block,/PX_PER_MM/);assert.match(block,/DOTS_PER_MM/);assert.match(block,/scaleX/);assert.match(block,/scrollHeight/);assert.equal(block.includes('setZoomFactor'),false);assert.match(script,/SetThreshold\(0\.58\)/);assert.match(script,/HighQualityBicubic/)});
